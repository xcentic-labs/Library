...
export async function PATCH(req: NextRequest) {
    try {
        const body = await req.json();
        const { userId, seatNumber, layoutId, timePeriod, razorpayOrderId, razorpayPaymentId, razorpaySignature , slot} = body as bookseat;

        if (!userId || !seatNumber || !layoutId || !timePeriod || !razorpayOrderId || !razorpayPaymentId || !razorpaySignature || !slot) return NextResponse.json({ "error": "All fields are required" }, { status: 400 });
