...
export const useLayout = () => {
  ...
  const [array, setArray] = useState(
    new Array(+layout.cols * +layout.rows).fill({
      isSeat: false,
      isBox: false,
    }),
  );
  ...
  const handleSeatPickUp = (index: number) => {
    ...
    setArray((prevArray) => {
      ...
      newArray[index] = {
        isSeat: selectedComponent == "seat" ? true : false,
        isBox: selectedComponent == "box" ? true : false,
      };
      return newArray;
    });
  };
  ...
  const handleApplyLayout = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!rowInput?.value || +rowInput.value == 0)
      return toast.error("enter no of rows");
    if (!colInput?.value || +colInput.value == 0)
      return toast.error("enter no of rows");
  ...
  const handleAddInput = () => {
    if (layoutDetails.MonthlyFee.length == 12)
      return toast.error("Can't add More then 12 Months");
  ...
  const handelNextStep = async () => {
    if (!layoutDetails.layoutName)
      return toast.error("Layout name is Required");
  ...
  const handleAddseats = async () => {
    ...
    if(duplicates.length > 0){
        return toast.error(
        `Duplicate Seat Numbers: ${duplicates.join(", ")}`,
      );
    }
    ...
    if (arr.length != 0)
      return toast.error(
        `Seat Number can't be empty at index ${arr[0]?.index}`,
      );
