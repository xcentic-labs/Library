            ...
            <div className='w-full bg-white rounded-lg flex flex-col justify-center items-start p-6 shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer border border-[#dad5be]'>
                <h3 className='text-xl font-semibold text-[#32524D] mb-4'>Seat Details</h3>
                {
                    data?.seat?.map((item, index) => (
                        <div key={index} className="w-full flex flex-col gap-3 mb-6 border-b border-[#dad5be] pb-4">
                            <p className="text-lg text-gray-700"><span className="font-bold text-[#32524D]">Layout Name : </span> {item.layout?.layoutName}</p>
                            <p className="text-lg text-gray-700"><span className="font-bold text-[#32524D]">Seat Number : </span> {item.seatNumber}</p>
                            <p className="text-lg text-gray-700"><span className="font-bold text-[#32524D]">Locker Available : </span> {item.isLocker  ? "Yes" : "No"}</p>
                            <p className="text-lg text-gray-700"><span className="font-bold text-[#32524D]">Booked For : </span> 
